   ...
   stage('Pushing Release to Nexus') {
    ...
    steps {
     container('maven') {
        ...
        checkout scm
        sh '''
        mvn clean install -Drevision=${BRANCH_NAME#*-} -s /usr/share/maven/ref/settings.xml
        mvn deploy -Dmaven.test.skip=true -Drevision=${BRANCH_NAME#*-} -s /usr/share/maven/ref/settings.xml
        '''
      }
      }
    }   
